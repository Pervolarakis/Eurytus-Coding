...
it('Successfully deletes pending request', async()=>{
    ...
    await request(app)
        .delete(`/api/v1/moderate/cancel/${pendingReq.id}`)
        .set('Cookie', global.signin(userId, 'user'))
        .expect(200)
    const deletedChallnge = await PendingRequest.findById(pendingReq.id);
    expect(deletedChallnge).toBeNull();
})
...
it('fails if challenge doesnt exist', async()=>{
    const userId = new mongoose.Types.ObjectId();
    await request(app)
        .delete(`/api/v1/moderate/cancel/${new mongoose.Types.ObjectId()}`)
        .set('Cookie', global.signin(new mongoose.Types.ObjectId(), 'user'))
        .expect(400)
})