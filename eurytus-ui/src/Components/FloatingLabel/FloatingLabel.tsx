...
const FloatingLabel = ({name, type, value, onChange}: FloatingLabelProps) => {
    return(
        <div className="relative m-2 mt-3">
            <input required id={name} name={name} type={type} value={value} onChange={(e)=>onChange(e.target.value)} className="peer h-10 w-full border-b-2 border-gray-300 text-gray-900 placeholder-transparent focus:border-secondary focus:outline-none" placeholder={name} />
