...
function Auth() {
  ...
  const [rightPanelActive, triggerRightPanelActive] = useState(false);
      ...
      <div className="bg-white hidden shadow-xl max-h-[600px] m-auto w-6/12 xl:w-8/12 2xl:w-7/12 3xl:w-6/12 lg:w-9/12 md:w-11/12 md:flex justify-center flex-column relative align-middle rounded-2xl overflow-hidden">
        ...
        <div className={`w-2/4 absolute left-0 h-full ${rightPanelActive? 'transform translate-x-full duration-500 opacity-0 z-0': 'transform -translate-x-0 duration-500 z-20 '}`}>
          <div className="h-full bg-secondary flex justify-center align-middle flex-col p-16">
            ...
            <div className="w-full flex justify-center">
              <button className="font-bold text-white border-white border-solid border p-2 rounded-2xl w-6/12 hover:bg-white hover:text-secondary" onClick={()=>triggerRightPanelActive(true)}>Sign in</button>
            </div>
          </div>
        </div>
        ...
        <div className={`w-2/4 absolute right-0 h-full ${rightPanelActive? 'transform translate-x-0 duration-500 z-20': 'transform -translate-x-full duration-500 opacity-0 '}`}>
          <div className="h-full bg-secondary flex justify-center align-middle flex-col p-16">
            ...
            <div className="w-full flex justify-center">
              <button className="font-bold text-white border-white border-solid border p-2 rounded-2xl w-6/12 hover:bg-white hover:text-secondary" onClick={()=>triggerRightPanelActive(false)}>Sign Up</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white shadow-xl min-h-[400px] m-auto sm:w-full flex justify-center flex-col relative align-middle rounded-2xl overflow-hidden">
          <div className="w-full flex justify-center">
            <div className="flex flex-row justify-around mt-12 shadow border w-72 rounded-3xl relative overflow-hidden">
              ...
              <div className={`w-1/2 h-full p-2 absolute left-0 z-10 rounded-3xl ${rightPanelActive?'bg-secondary transform translate-x-0 duration-500':'bg-secondary transform translate-x-full duration-500'}`}></div>
            </div>
          </div>
